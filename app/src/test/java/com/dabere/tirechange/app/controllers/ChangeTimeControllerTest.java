...
import com.dabere.tirechange.app.services.WorkshopsReader;
...
public class ChangeTimeControllerTest {
        ...
        @Autowired
        WorkshopsReader reader;

        @Test
        void testGetAllFreeTimes() throws Exception {
                MvcResult result = this.mockMvc.perform(MockMvcRequestBuilders.get("/appointments"))
        ...
        void testBookAnAppointment() throws Exception {
                Appointment appointment = tireChangeService.getAllAvailableTimes().get(0);
                MvcResult result = this.mockMvc
                                .perform(MockMvcRequestBuilders.put(
                                                "/appointments/{id}?workshopAddress=" + appointment.getWorkshopAddress(),
                                                appointment.getAppointmentId()))
                                .andExpect(status().isOk()).andReturn();
                String contentAsString = result.getResponse().getContentAsString();
                assertEquals("{\"message\":\"Broneering ̵nnestus.\"}", contentAsString);
